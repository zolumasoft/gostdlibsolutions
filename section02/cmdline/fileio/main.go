...
func deleteFile() {
	os.Create("file.txt") // Handle errors in production
	err := os.Remove("file.txt")
	if err != nil {
		log.Fatalln(err)
	}
	log.Printf("Deleted: file.txt\n")
}
...
func checkExistence() {
	...
	log.Printf("Exists. Size: %d and last modified %v\n", fi.Size(), fi.ModTime())
}

func renameFile() {
	f, _ := os.Create("file.txt") // Handle errors in production
...
func readWithScanner() {
	...
	for s.Scan() {
		lineNumber++
		log.Printf("%d - %s", lineNumber, s.Text())
	}
	if s.Err() != nil {
		log.Fatalln(err)
	}
