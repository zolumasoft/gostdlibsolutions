...
func find() {
	...
	fmt.Printf("FindAllStringIndex(%q): %v\n", s1, r.FindAllStringIndex(s1, -1))
	fmt.Printf("FindAllStringIndex(%q): %v\n", s2, r.FindAllStringIndex(s2, -1))
}
...
func split() {
	...
	s1Split := r.Split(s1, -1)
	s2Split := r.Split(s2, -1)
	s3Split := r.Split(s3, -1)

	fmt.Printf("Split - %v: %q\tLength: %d\n", s1, s1Split, len(s1Split))
	fmt.Printf("Split - %v: %q\tLength: %d\n", s2, s2Split, len(s2Split))
	fmt.Printf("Split - %v: %q\tLength: %d\n", s3, s3Split, len(s3Split))
}
